  ...
  <el-tab-pane label="Web账号配置" name="accoutManage">
    ...
    <el-card>
      <el-row class="management-table-scroll">
        <el-table class="management-table" :data="data.user_list" :show-overflow-tooltip="true">
          ...
          <el-table-column sortable prop="is_admin" label="是否管理员" width="130"></el-table-column>
          ...
          <el-table-column label="操作" width="180">
            <template #header>
              <div class="table-actions"><el-button type="primary" @click="addUser">新增</el-button>
              <el-button type="primary" @click="getUserList(0, 10000)">刷新</el-button></div>
            </template>
            <template #default="scope">
              <div class="table-actions"><el-button type="success" @click="editUser(scope.row)">编辑</el-button>
    ...
    <el-dialog title="用户信息" v-model="data.user_dialog_visible" width="60%">
      <el-form label-width="80px">
        ...
        <el-form-item label="管理员" prop="is_admin">
          <el-radio-group v-model="user.is_admin" id="is_admin">
...
function getUserList(offset: number = 0, limit: number = 10000) {
  axios.get<ResponseData>(`/api/user?offset=${offset}&limit=${limit}`)
    .then((ret) => {
      if (ret.data.code === 0) {
...
function updateUserById() {
  axios.put<ResponseData>(`/api/user`, getRequestBody())
    .then((ret) => {
      if (ret.data.code === 0) {
        data.user_list = ret.data.data;
      } else {
        ElMessage.error("更新用户错误,请检查输入");
      }
    })
}
...
function deleteUserById(userId: number) {
  axios.delete<ResponseData>(`/api/user/${userId}`)
    .then((ret) => {
      if (ret.data.code === 0) {
        data.user_list = ret.data.data;
      } else {
        ElMessage.error("删除用户错误");
      }
    })
}
 ...
 */
function createUser() {
  axios.post<ResponseData>(`/api/user`, getRequestBody())
    .then((ret) => {
      if (ret.data.code === 0) {
        data.user_list = ret.data.data;
      } else {
        ElMessage.error("创建用户错误,请检查输入");
      }
    })
}
